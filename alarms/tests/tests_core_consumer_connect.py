...
class TestConnectionToCoreConsumer:
    ...
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_connection_allowed_using_a_valid_password(self):
        """Test if clients can request the connection using a password"""
        # Arrange:
        AlarmCollection.reset([])
        query_string = 'password={}'.format(PROCESS_CONNECTION_PASS)
        # Connect:
        communicator = self.create_communicator(query_string=query_string)
        connected, subprotocol = await communicator.connect()
        assert connected, 'The communicator was not connected'
    ...
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_connection_not_allowed_for_an_invalid_password(self):
        """Test the connection can not be allowed with an invalid password"""
        # Arrange:
        query_string = 'password={}'.format(PROCESS_CONNECTION_PASS + '-fake')
        # Connect:
        communicator = self.create_communicator(query_string=query_string)
        connected, subprotocol = await communicator.connect()
        assert not connected, 'Connection should not be allowed'
